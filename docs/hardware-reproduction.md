...
本页收纳 HachimoDock 的硬件复刻资料，包括整机 BOM、结构件与装配、PCB 工艺信息。采购和打样前请结合工程文件再次核对型号、尺寸和供货状态。

硬件设计按 `CERN-OHL-S-2.0` 发布；基于硬件设计文件的修改和再分发请遵守该许可证及 [LICENSE.md](../LICENSE.md) 中的说明。
...
适合直接按底板工程与电子料 BOM 统一下单。若后续提供一键下单入口，请以嘉立创开源硬件项目页或仓库最新说明为准。
...
结构件模型随项目资料发布时提供；若后续同步到 MakerWorld 或其它模型平台，请以仓库最新链接为准。
