...
| 小屏已显示小狗，`http://<board-ip>/board-runtime-config.json` 可访问，但 manager 仍显示未连接 | 板子和 MQTT broker 正常，但电脑端 bridge HTTP 服务 `127.0.0.1:23333` 没有运行或刚被重启/杀掉 | 先查 `Get-NetTCPConnection -State Listen -LocalPort 1883,23333`；如果只有 `1883` 没有 `23333`，在仓库根目录重新运行 `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\start-hachimo-link.ps1 -LanIp <pc-lan-ip> -TargetSource codex`，再看 `http://127.0.0.1:23333/device-availability` |
...
如果第一条能返回 `brokerUrl`，且 `brokerUrl` 指向当前电脑的 `mqtt://<pc-lan-ip>:1883`，
但 `device-availability` 连接失败，通常不是板子问题，而是电脑端 bridge 没在监听
`23333`。此时只需要回仓库根目录重启本地 link：
