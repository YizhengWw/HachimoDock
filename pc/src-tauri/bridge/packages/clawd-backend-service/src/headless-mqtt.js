...
/*
 * [Input] Local agent state monitors, cumulative/per-turn token metadata, bridge profile env, and local Agent injection requests.
 * [Output] Parent-bound Bridge lifecycle with session-stable and current-day Agent token usage, hook/plugin-enriched Claude and MiMoCode session cards, atomically committed USB-forwarder state files, and agent-session injections with fresh-session recovery for stale Codex metadata.
 * [Pos] Headless status bridge for the Tauri Pet Manager runtime.
 * [Sync] If state-file, follow-source, or voice-injection recovery semantics change, update `pc/.folder.md`.
 */
...
class LocalStatePublisher {
  ...
  _writeLocalState(source, payload) {
    try {
      ...
      // Best-effort: the USB poller will use the next complete snapshot.
    }
  }
...
class HookHttpServer {
  ...
  handleRequest(req, res) {
    const { validateLocalRequest } = require("../../agent-session-bus/src/http-boundary");
    const boundaryError = validateLocalRequest(req);
    if (boundaryError) {
      this.sendJson(res, 403, { ok: false, error: boundaryError });
      return;
    }
...
async function main() {
  ...
  publisher = new LocalStatePublisher({
    enabledSources: config.localPublisher.enabledSources,
    selectedSource: config.localPublisher.selectedSource,
    localStateDir: config.localPublisher.localStateDir,
  });
  publisher.start();
