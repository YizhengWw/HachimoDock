 ...
 * vice versa. Claude Desktop's newest versioned Code runtime is supported
 * directly, while the Tauri-managed bridge also exports CLAUDE_CLI_PATH.
 */
class ClaudeCodeAdapter extends BaseAdapter {
  ...
  _defaultFallbackPaths() {
    ...
    return [
      ...macosClaudeDesktopCliFiles(this._env),
  ...
  _defaultExtraPathDirs() {
    ...
    return [
      ...macosClaudeDesktopCliFiles(this._env).map((candidate) => path.dirname(candidate)),
      `${home}/.npm-global/bin`,
      `${home}/.local/bin`,
      `${home}/.claude/local`,
...
function macosClaudeDesktopCliFiles(env, platform = process.platform) {
  if (platform !== "darwin") return [];
  const home = env.HOME || env.USERPROFILE || "";
  if (!home) return [];
  const runtimeRoot = path.join(home, "Library", "Application Support", "Claude", "claude-code");
  try {
...
module.exports = {
  ClaudeCodeAdapter,
  _internal: { macosClaudeDesktopCliFiles },
};
