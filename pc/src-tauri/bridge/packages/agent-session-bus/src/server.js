 ...
 * [Output] Loopback/Host/origin-validated Session snapshots, cursor events and streamed Agent injections with cleanup.
 * [Pos] HTTP boundary for Agent Session Bus.
 */

const crypto = require("crypto");
const http = require("http");
const { validateLocalRequest, isLoopback } = require("./http-boundary");
...
class AgentSessionBus {
  ...
  constructor({
    ...
    if (!isLoopback(host)) throw new Error("AgentSessionBus must bind to loopback");
    this.port = port;
    this.host = host;
  ...
  async _handle(req, res) {
    const boundaryError = validateLocalRequest(req, { allowAppOrigin: true });
    if (boundaryError) {
      sendJson(req, res, 403, { ok: false, error: boundaryError, code: "UNTRUSTED_ORIGIN" });
      return;
    }
    const url = new URL(req.url, `http://127.0.0.1:${this._actualPort || this.port}`);
