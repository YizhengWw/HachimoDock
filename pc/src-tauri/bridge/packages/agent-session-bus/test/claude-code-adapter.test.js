...
test("macOS resolver paths prefer the newest Claude Desktop managed Code runtime", async () => {
  await withFakeHome(null, async (home) => {
    for (const version of ["2.1.9", "2.1.10", "2.2.0"]) {
    ...
    const candidates = macosClaudeDesktopCliFiles({ HOME: home }, "darwin");
    assert.equal(candidates.length, 3);
    assert.match(candidates[0], /2\.2\.0[\\/]claude\.app[\\/]Contents[\\/]MacOS[\\/]claude$/);
    assert.match(candidates[1], /2\.1\.10[\\/]claude\.app[\\/]Contents[\\/]MacOS[\\/]claude$/);
  });
});
