...
// Keep them opt-in so we don't write invalid top-level keys into settings.json.
const ASSISTANT_HOOKS = [
  "AssistantMessage",
  "AssistantOutput",
];
...
function reconcileOptionalCommandHooks(settings, events, enabledSet) {
  let removed = 0;
  let changed = false;
...
function registerHooks(options = {}) {
  ...
  const assistantHooksEnabled = options.enableAssistantHooks !== undefined
    ? parseBooleanFlag(options.enableAssistantHooks)
    : parseBooleanFlag(process.env.CLAWD_ENABLE_ASSISTANT_HOOKS);
  const enabledAssistantEvents = assistantHooksEnabled ? new Set(ASSISTANT_HOOKS) : new Set();
  ...
  const optionalReconcileResult = reconcileOptionalCommandHooks(
    settings,
    ASSISTANT_HOOKS,
    enabledAssistantEvents
  );
  removed += optionalReconcileResult.removed;
  changed = changed || optionalReconcileResult.changed;

  // Build the full hook list: core + version-compatible hooks
  const hookEvents = [...CORE_HOOKS];
  for (const event of enabledAssistantEvents) {
  ...
  if (!options.silent) {
    ...
    if (!assistantHooksEnabled) {
      console.log("  Assistant hooks: disabled (set CLAWD_ENABLE_ASSISTANT_HOOKS=1 to opt in)");
    }
...
if (require.main === module) {
  try {
    ...
    const enableAssistantHooks = process.argv.includes("--assistant-hooks")
      || parseBooleanFlag(process.env.CLAWD_ENABLE_ASSISTANT_HOOKS);
    registerHooks({ remote, enableAssistantHooks });
  } catch (err) {
