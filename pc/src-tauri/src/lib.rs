 ...
 *          discovery including Finder-standard macOS application directories and
 *          Claude Desktop's newest bundled Code runtime, credential-preserving partial bridge-profile updates,
 ...
 * [Pos] Tauri runtime node in pc/src-tauri/src
...
// Public sources do not embed broker credentials. Runtime user settings remain supported.
const BUNDLED_MQTT_URL: Option<&str> = None;
const BUNDLED_MQTT_USERNAME: Option<&str> = None;
const BUNDLED_MQTT_PASSWORD: Option<&str> = None;
...
fn audio_bridge_signal(
    ...
    // Build the JSON object consumed by the USB-connected board runtime.
    ...
    if !usb_sent {
        return Err(usb_error
            .unwrap_or_else(|| "USB 未连接，无法向 ESP32-P4 下发板端音频命令。".to_string()));
    }
...
#[cfg(target_os = "macos")]
fn macos_application_directories(home: Option<&Path>) -> Vec<PathBuf> {
    ...
    if let Some(home_dir) = home {
        push_unique_path(&mut directories, home_dir.join("Applications"));
    }

    let search_paths = NSSearchPathForDirectoriesInDomains(
        NSSearchPathDirectory::ApplicationDirectory,
        NSSearchPathDomainMask::LocalDomainMask | NSSearchPathDomainMask::UserDomainMask,
        true,
    );
    for path in search_paths.iter() {
        push_unique_path(&mut directories, PathBuf::from(path.to_string()));
    }
...
fn macos_app_bundle_candidates(
    ...
    for applications_dir in macos_application_directories(home) {
        for app_name in app_names {
            push_unique_path(&mut candidates, applications_dir.join(app_name));
        }
    }
...
#[cfg(target_os = "macos")]
fn compare_numeric_version_labels(left: &str, right: &str) -> VersionOrdering {
    let parse = |value: &str| {
        value
            .split('.')
            .map(str::parse::<u64>)
...
#[cfg(target_os = "macos")]
fn macos_claude_desktop_cli_candidates(home: Option<&Path>) -> Vec<PathBuf> {
    let Some(home_dir) = home else {
        return Vec::new();
    };
    let runtime_root = home_dir
        .join("Library")
    ...
    let mut versions = entries
        ...
        .collect::<Vec<_>>();
    versions.sort_by(|(left, _), (right, _)| compare_numeric_version_labels(right, left));
    versions
        .into_iter()
        .map(|(_, path)| {
            path.join("claude.app")
                .join("Contents")
                .join("MacOS")
                .join("claude")
        })
...
fn find_claude_desktop_install(home: Option<&Path>) -> Option<String> {
    #[cfg(not(target_os = "macos"))]
    let _ = home;
...
fn find_claude_executable(home: Option<&Path>) -> Option<String> {
    #[cfg(target_os = "macos")]
    {
        for candidate in macos_claude_desktop_cli_candidates(home) {
            if candidate.is_file() {
                return Some(candidate.to_string_lossy().to_string());
            }
        }
...
fn detect_claude_code() -> DetectedAgent {
    ...
    let cli = find_claude_executable(home.as_deref());
    let desktop = find_claude_desktop_install(home.as_deref());
...
fn resolve_skill_source_dir(app: &tauri::AppHandle) -> Result<std::path::PathBuf, String> {
    ...
    Err(format!(
        "skill 源目录不存在；尝试过:\n  {}\n(production: tauri.conf.json bundle.resources 必须包含 skills/petui; debug: 确认 pc/skills/petui 存在)",
        tried.join("\n  ")
    ))
}
...
fn builtin_clawpkg_candidates(app_handle: &tauri::AppHandle, id: &str) -> Vec<PathBuf> {
    ...
    #[cfg(debug_assertions)]
    {
        ...
        if let Ok(current_dir) = env::current_dir() {
            ...
            push_unique_path(
                &mut candidates,
                current_dir.join("pc/builtin-clawpkgs").join(id),
            );
        }
...
fn compact_usb_daily_stats_payload(
    source: &str,
    payload: &serde_json::Value,
) -> Option<serde_json::Value> {
    let daily_usage = payload.get("dailyTokenUsage")?.as_object()?.clone();
    let ts_ms = payload
        .get("tsMs")
        .and_then(|value| value.as_u64())
...
fn start_usb_state_forwarder(usb_manager: usb_serial::UsbSerialManager) {
    thread::spawn(move || {
        ...
        let mut last_stats_signature = String::new();
        ...
        loop {
            ...
            let mut best_stats_payload: Option<serde_json::Value> = None;
            let mut best_stats_ts_ms = 0u64;
            ...
            for path in recent_bridge_json_paths(&state_dir, now_ms) {
                ...
                if let Ok(payload) = serde_json::from_str::<serde_json::Value>(&content) {
                    ...
                    let ts_ms = payload
                        ...
                        .and_then(|value| value.as_u64())
                        .unwrap_or(0);
                    if ts_ms >= best_stats_ts_ms {
                        if let Some(stats_payload) =
                            compact_usb_daily_stats_payload(&source, &payload)
                        {
                            best_stats_payload = Some(stats_payload);
                            best_stats_ts_ms = ts_ms;
                        }
                    }
            ...
            if let Some(stats_payload) = best_stats_payload {
                let stats_signature = serde_json::to_string(&stats_payload).unwrap_or_default();
                if stats_signature != last_stats_signature {
                    match usb_manager.send("runtime/stats", &stats_payload) {
                        Ok(_) => {
                            last_stats_signature = stats_signature;
                            eprintln!(
                                "[usb-forwarder] sent runtime/stats(source={})",
                                stats_payload
                                    .get("source")
                                    .and_then(|value| value.as_str())
                                    .unwrap_or("")
                        ...
                        Err(error) => {
                            eprintln!("[usb-forwarder] send runtime/stats error: {}", error)
                        }
                    }
                }
            }
...
fn write_launch_script(
    ...
    let codex_cli_export = find_codex_executable(get_home_dir().ok().as_deref())
        ...
        .unwrap_or_default();
    let claude_cli_export = find_claude_executable(get_home_dir().ok().as_deref())
        .map(|path| format!("export CLAUDE_CLI_PATH={}\n", shell_quote(&path)))
        .unwrap_or_default();
    ...
    let script = format!(
        ...
        claude_cli_export = claude_cli_export,
...
fn start_bridge_direct(
    ...
    if let Some(claude_cli_path) = find_claude_executable(get_home_dir().ok().as_deref()) {
        command.env("CLAUDE_CLI_PATH", claude_cli_path);
    }
...
mod tests {
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn macos_claude_discovery_prefers_newest_desktop_managed_runtime() {
        ...
        let runtime_root = home
            ...
            .join("Claude")
            .join("claude-code");
        for version in ["2.1.9", "2.1.10", "2.2.0"] {
            ...
            fs::write(executable, version).unwrap();
        }

        let candidates = macos_claude_desktop_cli_candidates(Some(home.path()));
        assert_eq!(
            candidates
                .first()
        ...
        assert_eq!(
            find_claude_executable(Some(home.path())).as_deref(),
            candidates.first().and_then(|path| path.to_str())
        );
    }

    #[cfg(target_os = "macos")]
    #[test]
    fn macos_agent_discovery_uses_user_and_system_application_directories() {
        let home = tempfile::tempdir().unwrap();
        let app = home.path().join("Applications").join("Claude.app");
        fs::create_dir_all(&app).unwrap();
        let directories = macos_application_directories(Some(home.path()));
        assert!(directories.contains(&home.path().join("Applications")));
        assert!(directories.iter().any(|path| {
            path.parent() == Some(Path::new(std::path::MAIN_SEPARATOR_STR))
                && path.file_name().and_then(|name| name.to_str()) == Some("Applications")
        }));
        assert_eq!(
            find_claude_desktop_install(Some(home.path())).as_deref(),
            app.to_str()
        );
    }
    ...
    #[test]
    fn compact_usb_daily_stats_is_session_independent() {
        let payload = serde_json::json!({
            "sessionId": "session-device-is-not-showing",
            "dailyTokenUsage": {
                "totalTokens": 42000,
                "inputTokens": 30000,
                "outputTokens": 12000,
        ...
        let compact = compact_usb_daily_stats_payload("codex", &payload).unwrap();
        assert_eq!(compact["source"], "codex");
        assert_eq!(compact["tokenUsage"]["totalTokens"], 42000);
        assert_eq!(compact["tokenUsage"]["inputTokens"], 30000);
        assert_eq!(compact["tokenUsage"]["outputTokens"], 12000);
        assert_eq!(compact["tokenUsagePeriod"], "today");
        assert!(compact.get("sessionId").is_none());
    }
    ...
    fn install_skill_into_agent_creates_skills_subdir() {
        ...
        std::fs::write(src.join("references/notes.md"), "pc").unwrap();
    ...
    fn windows_child_process_paths_remove_verbatim_prefixes_without_losing_unicode() {
        assert_eq!(
            child_process_path(Path::new(
                r"\\?\C:\Users\测试用户\AppData\Local\Pet Manager\bridge\runtime\node.exe"
            )),
            PathBuf::from(r"C:\Users\测试用户\AppData\Local\Pet Manager\bridge\runtime\node.exe")
        );
        assert_eq!(
