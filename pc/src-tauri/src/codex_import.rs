...
//! FFmpeg discovery prefers the packaged LGPL-only runtime and falls back to
//! the desktop host PATH resolver/current-user Windows package-manager roots.
//! H.264 output is restricted to the operating system encoder
//! (VideoToolbox/Media Foundation); GPL/nonfree external encoders are never
//! selected by Pet Manager.
//! Writes a manifest.json that matches the shape produced by
//! `pc/src/lib/appearance-store.js` so the gallery/detail UI can render it
...
#[allow(dead_code)] // Each release target constructs only its own OS encoder variant.
pub(crate) enum PlatformH264Encoder {
    VideoToolbox,
    MediaFoundation,
}

impl PlatformH264Encoder {
    ...
    pub(crate) fn append_quality_args(self, args: &mut Vec<String>) {
        match self {
            Self::VideoToolbox => args.extend([
                "-allow_sw".to_string(),
                "1".to_string(),
                "-realtime".to_string(),
                "0".to_string(),
...
fn expected_platform_h264_encoder() -> Result<PlatformH264Encoder, String> {
    #[cfg(target_os = "macos")]
    return Ok(PlatformH264Encoder::VideoToolbox);
    #[cfg(windows)]
    return Ok(PlatformH264Encoder::MediaFoundation);
    #[cfg(not(any(target_os = "macos", windows)))]
    Err("当前桌面平台没有受支持的系统 H.264 编码器".to_string())
}

pub(crate) fn platform_h264_encoder(ffmpeg: &str) -> Result<PlatformH264Encoder, String> {
    let expected = expected_platform_h264_encoder()?;
    let output = crate::command_for_host(ffmpeg)
        .args(["-hide_banner", "-encoders"])
        .output()
        .map_err(|error| format!("读取 FFmpeg 编码器列表失败: {error}"))?;
    ...
    if encoders.lines().any(|line| {
        line.split_whitespace()
            .any(|field| field == expected.codec_name())
    }) {
        return Ok(expected);
    }
    Err(format!(
        "FFmpeg 缺少当前系统所需的安全 H.264 编码器 {}",
        expected.codec_name()
    ))
}
...
pub(crate) fn resolve_ffmpeg() -> Result<String, String> {
    ...
    for candidate in ffmpeg_candidates() {
        if can_run_ffmpeg(&candidate) && platform_h264_encoder(&candidate).is_ok() {
            let _ = RESOLVED_FFMPEG.set(candidate.clone());
            return Ok(candidate);
        }
    }
    Err("未检测到包含系统 H.264 编码器的 FFmpeg 运行时".to_string())
}
...
pub fn import_codex_pet(
    ...
    let h264_encoder = platform_h264_encoder(&ffmpeg)?;
    ...
    for m in MAPPING {
        ...
        h264_encoder.append_quality_args(&mut encode_args);
        encode_args.extend([
            "-movflags".to_string(),
            "+faststart".to_string(),
