 ...
 * guards, accurate client-or-CLI discovery copy, SW1-confirm/SW3-back and joystick-up/down defaults and migration,
...
test("Agent and appearance card summarizes detected Agents and device bubble behavior", () => {
  ...
  assert.match(source, /已检测到 \$\{agentScan\.detectedCount\} 个 Agent，设备端仅显示当前跟随的 Agent 状态，对话结束后设备气泡保留显示 60 秒/);
});

test("current-Agent P4 session visibility is always on and follows adjacent snapshots", () => {
  const source = readSource("DeviceDashboard.jsx");
  ...
  assert.doesNotMatch(source, /loadSessionDisplayEnabled/);
  assert.doesNotMatch(source, /saveSessionDisplayEnabled/);
  ...
  assert.match(source, /useAgentSessionFeed\(\{[\s\S]*displayEnabled: true,/);
  assert.match(source, /useP4SessionSync\(\{[\s\S]*displayEnabled: true,/);
  assert.match(source, /<ChannelMatrixCard \/>/);
  assert.doesNotMatch(source, /showSessionDisplaySetting/);
  assert.doesNotMatch(source, /sessionDisplayEnabled/);
  assert.doesNotMatch(source, /onSessionDisplayEnabledChange/);
  assert.doesNotMatch(matrix, /import Switch/);
  assert.doesNotMatch(matrix, /SESSION_DISPLAY_COUNT_OPTIONS/);
  assert.doesNotMatch(matrix, /sessionDisplayCount/);
  assert.doesNotMatch(matrix, /仅显示当前运行中的 Agent 对话/);
...
test("dashboard keeps the client button map authoritative across reconnect and firmware OTA", () => {
  ...
  const p4Input = readRepoFile("..", "firmware", "main", "pet_p4_input.c");
  const p4Protocol = readRepoFile("..", "firmware", "main", "pet_p4_protocol.c");
...
test("Negative-screen proxy actions stay runtime-compatible without appearing in the P4 menu", () => {
  const dashboard = readSource("DeviceDashboard.jsx");
  const rust = readRepoFile("src-tauri", "src", "lib.rs");
  const firmware = readRepoFile("..", "firmware", "main", "pet_p4_input.c");
  const miniapp = readRepoFile("..", "firmware", "main", "pet_p4_miniapp.c");
  const p4Options = dashboard.match(/const P4_CUSTOM_ACTION_OPTIONS = \[([\s\S]*?)\];/);
...
test("appearance OTA syncs done and error WAV cues beside the matching videos", () => {
  ...
  assert.doesNotMatch(rust, /thread::sleep\(Duration::from_millis\(3\)\)/);
  assert.match(
    rust,
    /P4_CH343_RAW_WRITE_GAP: Duration = Duration::from_micros\(500\)/,
  );
