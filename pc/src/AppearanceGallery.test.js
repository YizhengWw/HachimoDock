 ...
 *          controls from the gallery, plus device-aware custom-appearance deletion.
...
test("gallery keeps desktop-pet assignment controls out of cards", () => {
  const gallery = readSource("AppearanceGallery.jsx");
  const css = readSource("styles.css");
...
test("gallery reads shared current-device state for active badges and safe deletion", () => {
  const gallery = readSource("AppearanceGallery.jsx");

  assert.match(gallery, /useDeviceContext\(\)/);
  assert.match(gallery, /currentDisplay,[\s\S]*deviceConnected,[\s\S]*applyDesktopPet,[\s\S]*refreshAppearances,[\s\S]*= useDeviceContext\(\)/);
...
test("custom appearance cards offer device-only or PC-and-device deletion", () => {
  const gallery = readSource("AppearanceGallery.jsx");
  const css = readSource("styles.css");

  assert.match(gallery, /deleteAppearance/);
  assert.match(gallery, /onRequestDelete=\{requestAppearanceDelete\}/);
  assert.match(gallery, /function AppearanceDeleteModal\(/);
  assert.match(gallery, /仅删除设备端/);
  assert.match(gallery, /PC 和设备都删除/);
  assert.match(gallery, /if \(isActiveOnDevice\) \{[\s\S]*await applyDesktopPet\([\s\S]*await deleteAppearance\(deleteTarget\.id\)/);
  assert.match(gallery, /row\.type === "builtin"/);
  assert.match(gallery, /!isBuiltin && \([\s\S]*aria-label=\{`删除形象 \$\{row\.name\}`\}/);
  assert.match(css, /\.appearance-card-actions\s*\{/);
  assert.match(css, /\.appearance-delete-confirm\s*\{/);
});
