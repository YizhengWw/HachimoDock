 ...
 *          component-scope/conflict guidance, single-slot replacement warning, consolidated deletion,
...
test("ComponentPreviewModal renders every action through the shared Button primitive", () => {
  ...
  assert.equal((source.match(/<Button\b/g) || []).length, 5);
...
test("ComponentPreviewModal exposes one consolidated delete entry when provided", () => {
  assert.doesNotMatch(source, /onRemove|Unplug|从设备移除/);
  assert.match(source, /typeof onDelete === "function"/);
