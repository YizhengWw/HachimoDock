 ...
 *          stable and config-versioned local macOS Accessibility packaging, Spotlight-independent macOS Agent application discovery with Claude Desktop's bundled Code runtime, relocatable bundled Node enforcement, source-matched P4 ready-cache reuse, audited LGPL-only platform-H.264 FFmpeg packaging with public corresponding-source release gates, binding-scoped appearance detail, Tauri local-media playback, and a release CSP without the removed browser-local background model surface.
...
test("desktop CSP no longer exposes the removed browser-local background model", () => {
  ...
  assert.doesNotMatch(csp, /staticimgly|imgly/i);
  assert.doesNotMatch(csp, /connect-src[^;]*https:\s/);
  assert.doesNotMatch(csp, /script-src[^;]*(?:blob:|'wasm-unsafe-eval')/);
});
...
test("release CSP and Cargo features exclude development-only privileges", () => {
  ...
  const publicBuildGuard = readSource("scripts/check-public-build-env.mjs");
  const provenanceGuard = readSource("scripts/check-clawd-provenance.mjs");
  const factoryReleasePackager = readSource("scripts/package-factory-release.mjs");
  ...
  assert.doesNotMatch(csp, /script-src[^;]*'wasm-unsafe-eval'/);
  ...
  assert.doesNotMatch(viteConfig, /cspSafeBackgroundRemovalPlugin|@imgly\/background-removal/);
  assert.match(packageJson.scripts["build:web"], /check:release-csp/);
  assert.match(packageJson.scripts["build:web"], /check:public-build-env/);
  assert.match(packageJson.scripts["prepare:desktop"], /check:public-build-env/);
  assert.match(packageJson.scripts.test, /check:provenance/);
  assert.match(provenanceGuard, /THIRD_PARTY_NOTICES\.md/);
  assert.match(provenanceGuard, /createHash\("sha256"\)/);
  assert.match(factoryReleasePackager, /hachimodock-p4-factory-flasher-v1/);
  assert.match(factoryReleasePackager, /flash-macOS\.command/);
  assert.match(factoryReleasePackager, /flash-Windows\.ps1/);
  assert.match(factoryReleasePackager, /contains a known internal credential/);
...
test("Tauri release resources are target-specific and package only audited LGPL FFmpeg", () => {
  const config = JSON.parse(readSource("src-tauri/tauri.conf.json"));
  ...
  assertResource(resources, "../../THIRD_PARTY_NOTICES.md", "THIRD_PARTY_NOTICES.md");
  assertResource(
    resources,
  ...
  assertResource(
    resources,
    "firmware/esp32-p4/firmware-macos.bin",
    "firmware/esp32-p4/firmware-macos.bin",
  );
  assertResource(
    resources,
  ...
  assertResource(macosConfig.bundle.resources, "generated-runtime/ffmpeg", "tools/ffmpeg");
  assertResource(macosConfig.bundle.resources, "generated-runtime/ffmpeg.LICENSE", "tools/ffmpeg.LICENSE");
  assertResource(macosConfig.bundle.resources, "generated-runtime/ffmpeg.README", "tools/ffmpeg.README");
  assertResource(macosConfig.bundle.resources, "generated-runtime/ffmpeg.SOURCE.txt", "tools/ffmpeg.SOURCE.txt");
  assertResource(macosConfig.bundle.resources, "generated-runtime/ffmpeg-8.1.2.tar.xz", "tools/source/ffmpeg-8.1.2.tar.xz");
  ...
  assert.match(localMacDmgBundler, /PET_MANAGER_DMG_FLAVOR/);
  assert.match(localMacDmgBundler, /\$\{productName\}_\$\{appVersion\}\$\{dmgFlavorSuffix\}_\$\{dmgArchitecture\}\.dmg/);
  ...
  assert.match(viteConfig, /__PET_MANAGER_INTERNAL_CONTENT_API_KEY__/);
  assert.doesNotMatch(viteConfig, /process\.env\.VITE_INTERNAL_CONTENT_API_KEY/);
  assertResource(
    windowsConfig.bundle.resources,
    "generated-runtime/node.exe",
    "bridge/runtime/node.exe",
  );
  assertResource(windowsConfig.bundle.resources, "generated-runtime/ffmpeg.exe", "tools/ffmpeg.exe");
  assertResource(windowsConfig.bundle.resources, "generated-runtime/ffmpeg.LICENSE", "tools/ffmpeg.LICENSE");
  assertResource(windowsConfig.bundle.resources, "generated-runtime/ffmpeg.README", "tools/ffmpeg.README");
  assertResource(windowsConfig.bundle.resources, "generated-runtime/ffmpeg.SOURCE.txt", "tools/ffmpeg.SOURCE.txt");
  assertResource(windowsConfig.bundle.resources, "generated-runtime/ffmpeg-8.1.2.tar.xz", "tools/source/ffmpeg-8.1.2.tar.xz");
  assertResource(windowsConfig.bundle.resources, "generated-runtime/zlib.LICENSE", "tools/zlib.LICENSE");
  ...
  assertTauriResourceSourceExists("firmware/esp32-p4/firmware-macos.bin");
  ...
  for (const retiredAgentWrapper of [
    "codebuddy.js",
    "copilot-cli.js",
    "cursor-agent.js",
    "gemini-cli.js",
    ...
    assert.equal(
      existsSync(join(refRoot, "src-tauri/bridge/agents", retiredAgentWrapper)),
      false,
      `retired broken Agent wrapper must not return: ${retiredAgentWrapper}`,
    );
  }
  ...
  assert.match(prepareScript, /PET_MANAGER_BUILD_FFMPEG_BIN/);
  assert.match(prepareScript, /ensureBundledFfmpeg/);
  assert.match(prepareScript, /--disable-gpl/);
  assert.match(prepareScript, /--disable-nonfree/);
  assert.match(prepareScript, /h264_videotoolbox/);
  assert.match(prepareScript, /h264_mf/);
  assert.match(prepareScript, /GNU LESSER GENERAL PUBLIC LICENSE/);
  assert.match(prepareScript, /PET_MANAGER_FFMPEG_SOURCE_ARCHIVE/);
  assert.match(prepareScript, /PET_MANAGER_ZLIB_LICENSE/);
  assert.match(prepareScript, /auditedFfmpegBinarySha256/);
  assert.match(prepareScript, /ffmpeg-8\.1\.2\.tar\.xz/);
  assert.match(prepareScript, /x86_64-w64-mingw32-objdump/);
  assert.doesNotMatch(prepareScript, /ffmpegStaticRelease|eugeneware|martin-riedl/);
  ...
  assert.match(ffmpeg, /bundled_ffmpeg_candidates|PET_MANAGER_BUNDLED_FFMPEG/);
  assert.match(ffmpeg, /h264_videotoolbox/);
  assert.match(ffmpeg, /h264_mf/);
  assert.doesNotMatch(ffmpeg, /"libx264"/);
  assert.match(ffmpeg, /find_executable\("ffmpeg", &\[\]\)/);
  assert.match(crossInstaller, /windowsFfmpeg/);
  assert.match(crossInstaller, /tools["', ]+ffmpeg\.exe/);
  assert.match(crossInstaller, /ffmpeg-8\.1\.2\.tar\.xz/);
  assert.match(
    crossInstaller,
    /join\(\s*stageRoot,\s*"firmware",\s*"esp32-p4",\s*"firmware\.bin",\s*\)/,
  );
  assert.match(
    crossInstaller,
  ...
  assert.match(
    crossInstaller,
    /assertExactFileCopy\(\s*bundledFirmwareSource,\s*bundledFirmwareDestination,/,
  );
  assert.match(crossInstaller, /const requiredResources = \[\s*bundledFirmwareDestination,/);
  ...
  assert.match(tauri, /manifest_dir\s*\.parent\(\) \/\/ pc\/\s*\.map\(\|p\| p\.join\("skills"\)\.join\(SKILL_NAME\)\)/);
  assertDebugOnlyFallback(
    tauri,
...
test("Windows installers stop only Pet Manager owned runtimes before replacing files", () => {
  const windowsConfig = JSON.parse(readSource("src-tauri/tauri.windows.conf.json"));
  const hooks = readSource("src-tauri/installer/windows/hooks.nsh");
  const stopScript = readSource("src-tauri/installer/windows/stop-owned-runtimes.ps1");
  const crossInstaller = readSource("scripts/build-windows-nsis-cross.mjs");

  assert.equal(
    windowsConfig.bundle.windows.nsis.installerHooks,
    "installer/windows/hooks.nsh",
  );
  assert.match(hooks, /NSIS_HOOK_PREINSTALL/);
  assert.match(hooks, /NSIS_HOOK_PREUNINSTALL/);
  assert.match(hooks, /PET_MANAGER_INSTALLER_HOOK_DIR/);
  assert.match(hooks, /pet-manager-stop-owned-runtimes\.ps1/);
  assert.match(stopScript, /\.ExecutablePath/);
  assert.match(stopScript, /bridge\\runtime\\node\.exe/);
  assert.match(stopScript, /pet-manager-tauri\.exe/);
  assert.match(stopScript, /Pet Manager\.exe/);
  assert.match(stopScript, /StringComparison\]::OrdinalIgnoreCase/);
  assert.doesNotMatch(stopScript, /taskkill\.exe[^\n]*\/IM[^\n]*node\.exe/i);
  assert.match(crossInstaller, /stop-owned-runtimes\.ps1/);
  assert.match(crossInstaller, /StopPetManagerOwnedRuntimes/);
});
...
test("coding-agent and media-tool detection uses user/environment-derived paths", () => {
  ...
  assert.match(tauri, /NSSearchPathForDirectoriesInDomains/);
  assert.match(tauri, /macos_claude_desktop_cli_candidates/);
  assert.match(tauri, /Application Support/);
  assert.match(tauri, /CLAUDE_CLI_PATH/);
  assert.match(claude, /macosClaudeDesktopCliFiles/);
