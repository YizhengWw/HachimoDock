 ...
 *          always-on current-Agent conversation visibility with active-only admission,
...
const P4_V7_DEFAULT_BUTTON_ACTIONS = {
...
export function mergeBoardButtonConfig(currentConfig = {}, response = {}, runtime = "") {
  const boardMigration = migrateP4V7ShippedBoardDefaults(response, runtime);
...
export default function DeviceDashboard({ binding, onUnbind, onOpenApiSettings }) {
  ...
  const toggleAudioBridge = useCallback(async (action) => {
    ...
    try {
      ...
      const transports = res?.usbSent ? "USB" : "";
      voiceDispatch({ type: "set_audio_bridge_state", enabled: requestedEnabled, ok: true, message: `已通过 ${transports || "USB"} 下发到板端` });
    } catch (err) {
  ...
  const onSendTest = useCallback(() => {
    const sendPromise = usb.connected
      ...
      : Promise.resolve({ ok: false, error: "USB 未连接" });
