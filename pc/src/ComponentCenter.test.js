 ...
 *          newest-first user components followed by explicitly ordered 双键接球 / 蛙蛙养成 builtins,
 ...
 *          P4 button-map downlink, semantic enabled state, repeatable card-owned device sync,
 *          consolidated device/local deletion, exact target identity, focus-safe confirmation,
...
test("ComponentCenter uses one library and removes the separate board overview", () => {
  ...
  assert.doesNotMatch(component, /component-center-header-tools/);
  ...
  assert.match(component, /component-library-toolbar/);
});
...
test("component center exposes a first-visit guide for direct card sync", () => {
  ...
  assert.match(component, /已同步组件可继续同步更新/);
  assert.match(component, /删除入口可选择仅从设备删除/);
...
test("component center explains the Agent Skill generation workflow below its title", () => {
  const component = readSource("ComponentCenter.jsx");
  assert.match(component, /title="组件中心"/);
  assert.match(
    component,
    /subtitle="点击右上方创建按钮为 Agent（如 ChatGPT 等）安装 Skill 后，可在 Agent 内使用该 Skill 生成组件（支持多轮优化调试），生成后组件会自动出现在组件中心"/,
  );
});

test("each component card keeps sync/update repeatable and routes removal through delete", () => {
  const component = readSource("ComponentCenter.jsx");
  ...
  assert.match(component, /onDeviceAction=\{\(\) => \{[\s\S]*if \(isInstalled\) syncSelectedComponent\(item\);[\s\S]*else setPreviewComponent\(item\)/);
  assert.doesNotMatch(component, /requestRemoveInventoryItem/);
  assert.match(component, /requestComponentDelete\(item, isInstalled\)/);
...
test("OTA, single-slot replacement guidance, and device-action confirmation remain while prompt modal is removed", () => {
  ...
  assert.match(component, /可以只删除设备上的副本/);
  assert.match(component, /Device removal happens first/);
...
test("library cards expose repeatable sync and consolidated device-first deletion", () => {
  const component = readSource("ComponentCenter.jsx");
  assert.match(component, /item\.isLocal \|\| isInstalled[\s\S]*requestComponentDelete\(item, isInstalled\)/);
  assert.match(component, /previewComponent\.isLocal \|\| previewIsInstalled[\s\S]*requestComponentDelete\(component, previewIsInstalled\)/);
  assert.match(component, /onDeviceAction=\{\(\) => \{/);
  assert.doesNotMatch(component, /requestRemoveInventoryItem/);
  assert.match(component, /仅从设备删除/);
  assert.match(component, /从电脑和设备删除/);
  assert.match(component, /type: "delete-options"[\s\S]*installedOnDevice/);
  assert.match(
    component,
...
test("device removal requires a live exact target and sends its board identity", () => {
  ...
  const requestBody = component.slice(
    component.indexOf("function requestComponentDelete"),
  ...
  assert.match(requestBody, /target: installedOnDevice \? liveInventoryTarget : null/);
...
test("builtin catalog starts with 双键接球 and a sprite-complete 蛙蛙养成", () => {
  ...
  const frogRoot = join(srcDir, "../builtin-clawpkgs/bloomfrog_companion");
  const frogManifest = JSON.parse(readFileSync(join(frogRoot, "component.json"), "utf8"));
  const frogWidget = JSON.parse(readFileSync(join(frogRoot, "runtime/widget.json"), "utf8"));
  assert.equal(frogManifest.name, "蛙蛙养成");
  assert.equal(frogManifest.version, "1.3.1");
  assert.equal(frogWidget.scene.sprites[0].asset, "assets/bloomfrog.png");
  assert.ok(readFileSync(join(frogRoot, "assets/bloomfrog.png")).length > 0);
});
...
test("component generation stays in the current Agent conversation through petui", () => {
  ...
  assert.match(component, /宠物养成小游戏，一个类似旅行青蛙的可爱小青蛙形象/);
  assert.match(component, /1键是抚摸、亲密度\+1/);
  assert.match(component, /2键是喂食、亲密度\+5/);
  assert.match(component, /摇杆上下左右可让它到处移动/);
  assert.match(component, /示意，请按照自己想法重新输入/);
...
test("component install keeps its button map package-owned without replacing device navigation", () => {
  const component = readSource("ComponentCenter.jsx");
  const tauri = readFileSync(join(srcDir, "../src-tauri/src/lib.rs"), "utf8");
  const firmware = readFileSync(join(srcDir, "../../firmware/main/pet_p4_input.c"), "utf8");
  const miniapp = readFileSync(join(srcDir, "../../firmware/main/pet_p4_miniapp.c"), "utf8");
