...
def smoke_test_tool_scene(
    ...
    while queue and len(visited) <= MAX_SEARCH_STATES:
        ...
        if len(path) >= min(MAX_SEARCH_DEPTH, 16):
            continue
        ...
        if effective_actions == set(simulator.actions) and (tick_advanced or not requires_ticks):
            break
    ...
    if missing_actions:
        errors.append(
            "以下互动工具输入在可达状态中没有产生即时可见反馈: "
            + ", ".join(missing_actions)
        )
    if requires_ticks and not tick_advanced:
        errors.append("互动工具声明了自动运动或 tick 规则，但可达状态中的画面没有推进")
    return {
        "ok": not errors,
        "componentId": manifest.get("id"),
        "interactiveToolScene": True,
...
def smoke_test_game(widget_dir: Path) -> dict[str, Any]:
    ...
    if manifest.get("kind") != "game":
        ...
        return {"ok": True, "skipped": True, "reason": "non-interactive tool has no scene"}
