...
static esp_err_t force_lcd_backlight_gpio(void) {
  ...
  ESP_RETURN_ON_ERROR(gpio_set_level(BSP_LCD_BACKLIGHT, LCD_BACKLIGHT_FORCE_LEVEL), TAG, "force LCD backlight GPIO");
  if (!g_forced_backlight_logged) {
    ESP_LOGI(TAG, "LCD backlight forced gpio=%d", LCD_BACKLIGHT_FORCE_LEVEL);
...
esp_err_t pet_p4_lcd_init(void) {
  ...
  esp_err_t force_err = force_lcd_backlight_gpio();
  if (g_backlight_status != ESP_OK || force_err != ESP_OK) {
    ESP_LOGW(TAG, "display backlight control failed: %s", esp_err_to_name(g_backlight_status));
  }
  if (g_backlight_status == ESP_OK && force_err != ESP_OK) {
...
esp_err_t pet_p4_lcd_keep_awake(void) {
  ...
  return g_backlight_status == ESP_OK ? ESP_OK : g_backlight_status;
}
