...
P4 的默认控制是：SW1 短按“确认进入下一级”、长按语音输入，SW2 短按打开组件中心，
SW3 短按“返回（取消）”；摇杆中键短按同样用于确认，长按默认不绑定，左推或右推切换
...
`buttons.json` 绑定，覆盖屏幕、SW1、SW2 和四向中按摇杆。退出不写入组件包，始终跟随
设备全局 `page_back` 设置（默认 SW3 短按）；旧包中的 `page_main/page_back` 下发时会被移除。
