...
P4 的默认控制是：SW1 长按语音输入，SW2 短按打开组件中心，SW3 短按
“返回（取消）”；旋钮短按“确认进入下一级”，长按默认不绑定，左旋或右旋切换
上一个/下一个会话。SW1/SW2/SW3 的短按与长按，以及旋钮短按、长按、左旋、
右旋都可以独立设置；按钮配置保存在 P4 NVS，设备重启后仍然有效。组件自己的
...
`component.json.kind` 必须明确为 `game` 或 `tool`。默认组件按产品顺序展示接住星星、
Flappy Bird、像素方块、像素贪吃蛇、番茄钟、喝水提醒和 Token 仪表盘。两类组件共用
`p4-bounded-runtime-v3` 受限声明式运行时；需要坐标、移动、碰撞或边界行为时再使用
`p4-grid-scene-v1` 通用场景能力。SW3 短按由设备保留为全局返回，新组件默认不绑定
旋钮长按；旧包中的 `page_main` 仅作兼容读取。内置组件保持产品顺序，正式本地组件按
...
`buttons.json` 绑定，覆盖屏幕、SW1-SW3 和旋钮。最终安装页可逐项调整；只要组件
声明了按键，安装时会把映射写入组件包，并且只在该组件打开后生效，
未声明实体事件会安全禁用，屏幕滑动保留为系统退出路径。调整结果按组件保存，
