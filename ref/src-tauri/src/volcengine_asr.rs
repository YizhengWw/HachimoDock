/*
 * [Input] ESP32-P4 microphone PCM (16 kHz mono S16LE) and a Volcengine API key from environment, Windows Credential Manager, or the macOS app-private config file.
 * [Output] Platform-specific credential persistence plus VAD-segmented streaming partial/final transcripts using the BigModel ASR WebSocket API.
...
pub fn settings_status() -> Result<DeviceAsrSettingsStatus, String> {
    ...
    match read_stored_config()? {
        Some(config) if !config.api_key.trim().is_empty() => Ok(public_status(
            true,
...
fn load_runtime_config() -> Result<RuntimeAsrConfig, String> {
    ...
    let config =
        read_stored_config()?.ok_or_else(|| "未配置火山引擎语音识别 API Key".to_string())?;
    let api_key = config.api_key.trim();
    if api_key.is_empty() {
        return Err("未配置火山引擎语音识别 API Key".to_string());
    }
    Ok(RuntimeAsrConfig {
        api_key: api_key.to_string(),
