 ...
 * [Output] Static Node regression coverage for Volcano Ark API-key-only setup, 1.5-first defaults,
...
test("Volcano generation setup only asks for API key plus a dropdown model name", () => {
  ...
  assert.match(wizard, /doubao-seedance-1-5-pro-251215/);
  assert.match(wizard, /DEFAULT_VOLCANO_VIDEO_MODEL/);
  assert.match(
    wizard,
