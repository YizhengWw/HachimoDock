...
| `DeviceContext.jsx` | `component` | Single source for binding/USB status/independent wifiOnline/deviceOnline/appearances/agentAppearanceMap/enabledAgents/agentOptions polling; it hydrates active channel selection from `load_bridge_profile` before trusting localStorage cache, keeps background serial auto-connect in Rust, and exposes manual `rescanUsbDevices()` for user-triggered scan/connect recovery. Exposes `useDeviceContext()` and pure helper `deriveCurrentDisplay` for tests |
| `DeviceContext.test.js` | `test` | Runtime Node coverage for `deriveCurrentDisplay` across bound/unbound/stale-channel cases + static checks that Provider wires the documented context value shape, hydrates bridge-profile active channel state, keeps USB and WiFi availability separate, keeps polling from auto-connecting, and exposes manual USB serial rescan/connect |
