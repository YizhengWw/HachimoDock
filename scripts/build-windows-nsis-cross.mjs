/**
 * [Input] A cargo-xwin-built Pet Manager x64 executable and the prepared release resource sources.
 * [Output] A current-user NSIS installer with install-relative Node/FFmpeg resources and an embedded WebView2 bootstrapper.
...
const refRoot = join(repositoryRoot, "ref");
...
assertPeExecutable(windowsFfmpeg, "Windows FFmpeg runtime");
...
for (const notice of ["ffmpeg.LICENSE", "ffmpeg.README", "ffmpeg.SOURCE.txt"]) {
  copyFileSync(
...
copyTree(
  join(repositoryRoot, "skills", "petui"),
