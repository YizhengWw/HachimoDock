"""
[Input] PetUI validator/simulator sources plus bounded game/tool fixtures and PNG assets.
[Output] Regression coverage for structural validation, sprite contracts, playable games,
and observable interaction across scene-based tools.
[Pos] Host-side PetUI generation gate tests for the ESP32-P4 runtime.
[Sync] If this file changes, update firmware/.folder.md and skills/.folder.md.
"""
...
VALIDATOR_PATH = (
    WORKSPACE_ROOT / "pc" / "skills" / "petui" / "scripts" / "validate_generated_widget.py"
)
...
SMOKE_PATH = (
    WORKSPACE_ROOT / "pc" / "skills" / "petui" / "scripts" / "smoke_test_widget_game.py"
)
...
class GameplaySmokeTests(unittest.TestCase):
    ...
    def test_interactive_tool_scene_exercises_every_action_and_tick(self) -> None:
        package = WORKSPACE_ROOT / "pc" / "builtin-clawpkgs" / "bloomfrog_companion"
        result = run_smoke_test(package)
        self.assertTrue(result["ok"], result)
        self.assertTrue(result["interactiveToolScene"], result)
        self.assertEqual(
            result["effectiveActions"],
            ["feed_pet", "move_down", "move_left", "move_right", "move_up", "pet_pet"],
        )
        self.assertTrue(result["tickAdvanced"], result)
        ...
        for relative in (
            ...
            values[relative] = json.loads((package / relative).read_text(encoding="utf-8"))
        values["assets/bloomfrog.png"] = (package / "assets" / "bloomfrog.png").read_bytes()
        runtime = values["runtime/widget.json"]
        assert isinstance(runtime, dict)
        scene = runtime["scene"]
        assert isinstance(scene, dict)
        rules = scene["rules"]
        assert isinstance(rules, list)
        scene["rules"] = [rule for rule in rules if rule.get("on") != "feed_pet"]
        transitions = runtime["transitions"]
        assert isinstance(transitions, list)
        for transition in transitions:
            if transition.get("on") == "feed_pet":
                transition.pop("inc", None)
                transition.pop("set", None)
        broken = self.smoke_values(values)
        self.assertFalse(broken["ok"], broken)
        self.assertTrue(any("feed_pet" in error for error in broken["errors"]), broken)
