...
Also locks Session-independent daily Token telemetry and cached-stat replay on mini-app activation.
Also locks selected-Session lifecycle authority for appearance playback.
...
Also locks interrupted OTA recovery, visible transfer feedback, the component-runtime
indicator-free viewport, and sprite-complete built-in packages on the device.
...
def read_usb_serial_contract():
    sources = [
        "pc/src-tauri/src/usb_serial.rs",
        "pc/src-tauri/src/usb_serial/appearance_transaction.rs",
        "pc/src-tauri/src/usb_serial/connection_handle.rs",
        "pc/src-tauri/src/usb_serial/firmware_transaction.rs",
        "pc/src-tauri/src/usb_serial/native_usb_protocol.rs",
        "pc/src-tauri/src/usb_serial/transfer_log.rs",
        "pc/src-tauri/src/usb_serial/transaction_waiters.rs",
        "pc/src-tauri/src/usb_serial/widget_transaction.rs",
    ]
...
def test_pc_p4_sync_prefers_native_usb_bulk_transport():
    cargo = read_workspace("pc/src-tauri/Cargo.toml")
    usb = read_usb_serial_contract()
    native_protocol = read_workspace(
        "pc/src-tauri/src/usb_serial/native_usb_protocol.rs"
    )
    lib = read_workspace("pc/src-tauri/src/lib.rs")
...
def test_p4_bounded_miniapp_contract_is_explicit_and_heap_safe():
    ...
    cmake = read("main/CMakeLists.txt")
    desktop_preview = read_workspace("pc/src/component-center/DeviceScreenPreview.jsx")
    desktop_styles = read_workspace("pc/src/styles.css")
...
def test_p4_token_widget_uses_bounded_stats_instead_of_host_readers():
    ...
    protocol = read("main/pet_p4_protocol.c")
    input_runtime = read("main/pet_p4_input.c")
    desktop = read_usb_serial_contract()
    desktop_lib = read_workspace("pc/src-tauri/src/lib.rs")
    codex_monitor = read_workspace("pc/src-tauri/bridge/agents/codex-log-monitor.js")
    claude_monitor = read_workspace("pc/src-tauri/bridge/agents/claude-log-monitor.js")
    ...
    activation = input_runtime[input_runtime.index('strcmp(binding->action, "page_enter") == 0') :]
    activation = activation[: activation.index('strcmp(binding->action, "page_back") == 0')]
    assert "pet_p4_miniapp_catalog_activate_selected()" in activation
    assert "pet_p4_miniapp_sync_stats(&state->stats)" in activation


def test_builtin_tool_widgets_declare_bounded_completion_cycles():
    miniapp = read_required("main/pet_p4_miniapp.c")
    tomato = json.loads(read_workspace("pc/builtin-clawpkgs/tomato-clock/runtime/widget.json"))
    token_usage = json.loads(read_workspace("pc/builtin-clawpkgs/token-usage/runtime/widget.json"))
...
def test_builtin_tool_widgets_use_global_exit_and_fit_p4_limits():
    ...
    for widget_id, expected_kind in kinds.items():
        manifest = json.loads(
            read_workspace(f"pc/builtin-clawpkgs/{widget_id}/component.json")
        )
        assert manifest["kind"] == expected_kind

    for widget_id, actions in expected_actions.items():
        runtime = json.loads(
            read_workspace(f"pc/builtin-clawpkgs/{widget_id}/runtime/widget.json")
        )
        buttons = json.loads(
            read_workspace(f"pc/builtin-clawpkgs/{widget_id}/buttons.json")
        )
    ...
    drink = json.loads(
        read_workspace("pc/builtin-clawpkgs/drink-reminder/runtime/widget.json")
    )
    ...
    token = json.loads(
        read_workspace("pc/builtin-clawpkgs/token-usage/runtime/widget.json")
    )
...
def test_p4_component_packages_commit_through_validated_ab_catalog_snapshots():
    ...
    assert '"spritesChecksum"' in miniapp
    ...
    assert "compile_component_sprites(package_dir, widget)" in factory
    assert '"spritesChecksum": f"{sprite_pack_checksum(sprites):08x}"' in factory
...
def test_p4_ab_firmware_embeds_and_migrates_all_builtin_components():
    ...
    for item in bundle["components"]:
        ...
        assert item["widget"] == json.loads(
            read_workspace(f"pc/builtin-clawpkgs/{component_id}/runtime/widget.json")
        )
        assert item["buttons"] == json.loads(
            read_workspace(f"pc/builtin-clawpkgs/{component_id}/buttons.json")
        )
        assert re.fullmatch(r"[0-9a-f]{8}", item["spritesChecksum"])
        assert isinstance(item["spriteFiles"], list)
    frog = next(item for item in bundle["components"] if item["id"] == "bloomfrog_companion")
    assert [sprite["id"] for sprite in frog["spriteFiles"]] == ["bloomfrog"]
    assert all(sprite["data"] for sprite in frog["spriteFiles"])
    ...
    assert "User-created packages" in protocol_doc
    assert "`bloomfrog_companion`" in protocol_doc
...
def test_builtin_pixel_games_use_bounded_native_game_presets():
    ...
    blocks = json.loads(read_workspace("pc/builtin-clawpkgs/block-combo/runtime/widget.json"))
    snake = json.loads(read_workspace("pc/builtin-clawpkgs/snake-turn/runtime/widget.json"))
    flappy = json.loads(read_workspace("pc/builtin-clawpkgs/flappy-bird/runtime/widget.json"))
    ...
    block_buttons = json.loads(
        read_workspace("pc/builtin-clawpkgs/block-combo/buttons.json")
    )
    assert {
    ...
    flappy_buttons = json.loads(
        read_workspace("pc/builtin-clawpkgs/flappy-bird/buttons.json")
    )
...
def test_generic_scene_runtime_is_shared_by_games_and_tools():
    ...
    two_key_pong = json.loads(
        read_workspace(
            "pc/builtin-clawpkgs/two-key-pong/runtime/widget.json"
        )
    )
    tomato = json.loads(
        read_workspace(
            "pc/builtin-clawpkgs/tomato-clock/runtime/widget.json"
        )
    )
...
def test_petui_skill_contains_no_copyable_component_packages():
    skill_root = WORKSPACE / "pc" / "skills" / "petui"
...
def test_p4_identity_and_protocol_nack_are_explicit():
    ...
    desktop = read_workspace("pc/src-tauri/src/lib.rs")
...
def test_p4_stats_model_feeds_widgets_without_a_fixed_stats_page():
    ...
    assert 'main_open ? "components"' in touch
    assert 'strcmp(state->screen_page, "app") == 0 ? "components"' in touch
...
def test_p4_conversation_queue_is_synced_and_rendered_with_pixel_ellipsis():
    ...
    assert "static const char *selected_session_lifecycle(" in protocol
    assert "state->session_queue[selected].state" in protocol
    effective_lifecycle = protocol[
        protocol.index("const char *pet_p4_state_effective_lifecycle("):
        protocol.index("bool pet_p4_state_touch_feedback_active(")
    ]
    assert "selected_session_lifecycle(state)" in effective_lifecycle
    assert "if (session_lifecycle) lifecycle = session_lifecycle" in effective_lifecycle
...
def test_p4_runtime_supports_native_usb_and_usb_uart_bridge():
    ...
    transport_config = read("main/pet_p4_transport_config.h")
    platformio = read("platformio.ini")
    doc = read("README.md")
...
def test_pc_uses_high_baud_for_p4_ch343_usb_uart():
    ...
    assert "P4_RAW_APPEARANCE_ASSET_CHUNK_SIZE: usize = 64 * 1024" in usb
    assert "P4_RAW_APPEARANCE_ASSET_CHUNK_SIZE: usize = P4_RAW_APPEARANCE_SAFE_CHUNK_SIZE" in usb
    assert "P4_RAW_APPEARANCE_CHUNK_MAX_ATTEMPTS: u32 = 4" in usb
    assert "P4_CH343_SERIAL_WRITE_SLICE_BYTES: usize = 64" in usb
    assert "P4_CH343_SERIAL_WRITE_GAP: Duration = Duration::from_micros(400)" in usb
    assert "P4_CH343_SERIAL_WRITE_GAP: Duration = Duration::from_micros(250)" in usb
    assert "P4_CH343_RAW_WRITE_SLICE_BYTES: usize = 128" in usb
    assert "P4_CH343_RAW_WRITE_GAP: Duration = Duration::from_micros(500)" in usb
...
def test_p4_h264_uses_the_v9_single_slice_full_duration_aspect_fit_contract():
    ...
    desktop = read_usb_serial_contract()
    prebuilder = read_workspace("pc/scripts/prepare-p4-ready-assets.mjs")
    ...
    # The shipped runtime now uses OS H.264 encoders, not libx264 options.
    encoder = read_workspace("pc/src-tauri/src/codex_import.rs")
    assert "h264_videotoolbox" in encoder
    assert "h264_mf" in encoder
    assert "exactly one slice per access unit" in desktop
    assert "p4_h264_accepts_native_baseline_sps_and_rejects_unsupported_profiles" in desktop
...
def test_saved_and_codex_imported_appearances_are_prepared_before_sync():
    desktop_lib = read_workspace("pc/src-tauri/src/lib.rs")
    appearance_store = read_workspace("pc/src/lib/appearance-store.js")
    gallery = read_workspace("pc/src/AppearanceGallery.jsx")
...
def test_p4_flash_layout_allocates_dual_10m_appearance_slots_on_32m_flash():
    ...
    assert "| `ota_0` | `0x010000` | 2.5 MiB |" in runtime_readme
    assert "| `ota_1` | `0x290000` | 2.5 MiB |" in runtime_readme
    assert "| `storage` | `0x520000` | 6.875 MiB |" in runtime_readme
    assert "| `appearance0` | `0xC00000` | 10 MiB |" in runtime_readme
    assert "| `appearance1` | `0x1600000` | 10 MiB |" in runtime_readme
    ...
    assert "旧文件系统不会原样写回较小的新分区" in runtime_readme
...
def test_p4_ab_firmware_ota_is_verified_acknowledged_and_exposed_by_pc():
    ...
    desktop = read_usb_serial_contract()
    desktop_lib = read_workspace("pc/src-tauri/src/lib.rs")
    desktop_ui = read_workspace("pc/src/dashboard/FirmwareUpdateModal.jsx")
    bundled_ui = read_workspace("pc/src/shell/FirmwareUpdateNavItem.jsx")
    ...
    pacing = read_workspace("pc/src-tauri/src/usb_serial.rs")
    assert "P4_CH343_SERIAL_WRITE_SLICE_BYTES: usize = 64" in pacing
    assert "P4_CH343_RAW_WRITE_SLICE_BYTES: usize = 128" in pacing
    ...
    assert "payload corruption at seq=" in read_workspace("pc/src-tauri/src/usb_serial.rs")
...
def test_pc_and_p4_build_identity_share_protocol_schema_and_diagnostics():
    ...
    protocol_doc = read("protocol.md")
    runtime_readme = read("README.md")
    desktop_build = read_workspace("pc/src-tauri/build.rs")
    desktop_serial = read_usb_serial_contract()
    desktop_lib = read_workspace("pc/src-tauri/src/lib.rs")
    desktop_ui = read_workspace("pc/src/dashboard/DeviceDiagnosticsModal.jsx")
...
def test_p4_component_lock_uses_platformio_exported_portable_root():
    ...
    assert "def normalize_component_lock(source, target, env):" in platformio_hook
...
def test_p4_raw_asset_chunks_bypass_base64_with_bounded_ram_and_checksum_ack():
    ...
    desktop = read_usb_serial_contract()
    assert "P4_RAW_APPEARANCE_ASSET_CHUNK_SIZE: usize = 64 * 1024" in desktop
    assert "P4_RAW_APPEARANCE_ASSET_CHUNK_SIZE: usize = P4_RAW_APPEARANCE_SAFE_CHUNK_SIZE" in desktop
...
def test_p4_device_ptt_stream_is_validated_and_relayed_by_pc():
    ...
    desktop = read_workspace("pc/src-tauri/src/usb_audio.rs")
    desktop_lib = read_workspace("pc/src-tauri/src/lib.rs")
...
def test_windows_tooling_forces_utf8_and_ascii_paths():
    shared = read_workspace("pc/scripts/windows-utf8.ps1")
    p4_tool = read_required("tools/p4.ps1")
    migration = read_required("tools/migrate_to_ab.ps1")
    dev_launcher = read_workspace("pc/scripts/dev-manager.ps1")
    open_manager = read_workspace("pc/scripts/open-manager.ps1")
