...
当前公开构建 0.7.64-p4：v1/v3 实时对话固定状态文案去掉省略号，避免设备字库显示为问号；正常对话字幕保留标点。PC 0.1.82 公开包内置两个芯片版本的应用固件，升级保留形象、组件和配置。
...
默认顺序为：**自选股行情、双键接球、蛙蛙养成、Flappy Bird、方块游戏、贪吃蛇、番茄钟、喝水提醒、Token 仪表盘**。

自选股行情是默认列表的第一项，**不是固定置顶项**；用户新安装的组件会排在默认组件前面。设备开机进入宠物主界面，组件中心初始选择自选股行情。
