/*
 * Local 480x640 desk-pet display BSP adaptations.
 * The MIPI command return path is treated as write-only during panel startup.
 */
#include "sdkconfig.h"
...
esp_codec_dev_handle_t bsp_audio_codec_es8311_duplex_init(void)
{
    if (i2s_data_if == NULL)
    {
        BSP_ERROR_CHECK_RETURN_NULL(bsp_i2c_init());
...
esp_err_t bsp_display_new_with_handles(const bsp_display_config_t *config, bsp_lcd_handles_t *ret_handles)
{
    ...
    // This panel's DSI return path is not wired. ACK requests leave completed
    // writes pending until the command FIFO fills and stalls panel init.
    mipi_dsi_host_ll_enable_cmd_ack(MIPI_DSI_LL_GET_HOST(bus_config.bus_id), false);
    ESP_LOGI(TAG, "MIPI DSI command ACK disabled for write-only panel link");
