#!/usr/bin/env python3
"""
[Input] PlatformIO artifacts, factory-config.json, desktop-built Terrier P4 ready pack, and sprite-complete P4 components.
[Output] A preloaded SPIFFS image and one flashable ESP32-P4 factory binary.
[Pos] Factory provisioning tool for firmware.
...
def find_project_dir(build_dir: Path, explicit: str | None) -> Path:
    ...
    for candidate in [build_dir.resolve(), *build_dir.resolve().parents]:
        if (candidate / "platformio.ini").is_file():
            return candidate
    raise FileNotFoundError("could not locate firmware from the build directory")
...
def build_builtin_component_tree(
    ...
    for slot, widget_id_value in enumerate(component_ids):
        ...
        sprites = compile_component_sprites(package_dir, widget)
        ...
        for sprite_index, sprite in enumerate(sprites):
            write_spiffs_root_file(
                spiffs_tree,
                f"p4s{slot:02d}-0-{sprite_index}.bin",
                sprite["data"],
            )
            package_bytes += len(sprite["data"])
            sprite_file_count += 1
    ...
    return {
        ...
        "physicalFiles": len(catalog_items) * 2 + sprite_file_count + 1,
        "spriteFiles": sprite_file_count,
