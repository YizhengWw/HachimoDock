...
def test_component_renderer_applies_classic_palette_and_scoreboard_presets() -> None:
    root = Path(__file__).resolve().parents[1]
    renderer = root / "fb-stats-renderer.py"
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        rendered = []
        for palette, layout in [
            ("sunset", "arcade"),
            ("mint", "arcade"),
            ("arcade", "scoreboard"),
        ]:
            payload = tmp_path / f"{palette}.display"
            frame = tmp_path / f"{palette}.rgb565"
            payload.write_text(
                "\n".join(
                    [
                        ...
                        "note=跨端安全预置",
                        "footer=SW1 · SW2 · SW3",
                        f"visualPalette={palette}",
                        f"visualLayout={layout}",
                    ]
                ),
                encoding="utf-8",
            )
        ...
        assert len({bytes(data) for data in rendered}) == 3
...
if __name__ == "__main__":
    ...
    test_component_renderer_applies_classic_palette_and_scoreboard_presets()
