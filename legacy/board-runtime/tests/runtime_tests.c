...
static void test_runtime_stats_dashboard_payload_for_framebuffer_renderer(void) {
  ...
  assert_true(strstr(display, "tokenTotal=1.30M") != NULL, "stable total reader field");
  assert_true(strstr(display, "tokenInput=900.0K") != NULL, "stable input reader field");
  assert_true(strstr(display, "tokenOutput=400.0K") != NULL, "stable output reader field");
  assert_true(strstr(display, "tokenCache=缓存 0") != NULL, "stable cache reader field");
