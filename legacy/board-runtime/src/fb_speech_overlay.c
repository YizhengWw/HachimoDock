...
static void br_component_dashboard_defaults(br_component_dashboard_model *model) {
  ...
  snprintf(model->visual_style, sizeof(model->visual_style), "classic");
  snprintf(model->visual_palette, sizeof(model->visual_palette), "candy");
  snprintf(model->visual_layout, sizeof(model->visual_layout), "arcade");
  snprintf(model->visual_sprite, sizeof(model->visual_sprite), "target");
}
...
static void br_component_dashboard_set_value(br_component_dashboard_model *model, const char *key, const char *value) {
  ...
  } else if (strcmp(key, "visualStyle") == 0) {
    br_normalize_text(value, "classic", model->visual_style, sizeof(model->visual_style));
  } else if (strcmp(key, "visualPalette") == 0) {
    br_normalize_text(value, "candy", model->visual_palette, sizeof(model->visual_palette));
  } else if (strcmp(key, "visualLayout") == 0) {
    br_normalize_text(value, "arcade", model->visual_layout, sizeof(model->visual_layout));
  } else if (strcmp(key, "visualSprite") == 0) {
    br_normalize_text(value, "target", model->visual_sprite, sizeof(model->visual_sprite));
  }
}
...
static bool br_overlay_render_component_dashboard(
  ...
  bool compact = fb->height <= 540;
  bool pixel = strcmp(model->visual_style, "pixel") == 0;
  bool pixel_tool = pixel && strcmp(model->visual_layout, "tool") == 0;
  uint32_t bg = br_pack_pixel(5, 7, 6, 255);
  uint32_t scanline = br_pack_pixel(16, 27, 19, 255);
  uint32_t green_wash = br_pack_pixel(10, 31, 22, 255);
  uint32_t amber_wash = br_pack_pixel(36, 25, 10, 255);
  ...
  uint32_t green = br_pack_pixel(85, 193, 133, 255);
  uint32_t dark_text = br_pack_pixel(19, 18, 13, 255);
  uint32_t title_color = br_pack_pixel(225, 241, 231, 255);
  uint32_t progress_track = br_pack_pixel(40, 40, 40, 255);
  uint32_t note_color = br_pack_pixel(164, 169, 160, 255);

  if (pixel_tool) {
    uint32_t accent = br_pack_pixel(246, 183, 60, 255);
    if (strcmp(model->visual_palette, "sunset") == 0) {
      accent = br_pack_pixel(255, 102, 92, 255);
    } else if (strcmp(model->visual_palette, "mint") == 0) {
      accent = br_pack_pixel(87, 217, 255, 255);
    } else if (strcmp(model->visual_palette, "arcade") == 0) {
      accent = br_pack_pixel(170, 140, 255, 255);
    }
    orange = accent;
    green = accent;
  } else if (pixel) {
    bg = br_pack_pixel(90, 215, 255, 255);
    scanline = br_pack_pixel(112, 225, 255, 255);
    green_wash = bg;
    amber_wash = bg;
    panel = br_pack_pixel(255, 243, 184, 255);
    dim = br_pack_pixel(43, 35, 103, 255);
    ivory = br_pack_pixel(255, 79, 139, 255);
    orange = br_pack_pixel(255, 79, 139, 255);
    green = br_pack_pixel(255, 216, 61, 255);
    dark_text = br_pack_pixel(43, 35, 103, 255);
    title_color = dark_text;
    progress_track = br_pack_pixel(43, 35, 103, 255);
    note_color = dim;
  }
  ...
  if (model->progress[0]) {
    ...
    if (pct >= 0) {
      ...
      br_fill_rect(fb, bar_x, bar_y, bar_w, bar_h, progress_track);
  ...
  if (model->footer[0]) {
    ...
    br_draw_scaled_text_fit(config, fb, cache, model->footer, margin, footer_y,
                            ...
                            pixel_tool ? orange : br_pack_pixel(111, 183, 142, 255));
  }
  return true;
}
