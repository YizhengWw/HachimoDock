/*
 * [Input] Normalized per-agent token usage frames and persisted daily/session aggregates.
 * [Output] Daily token snapshots plus STATS_DASHBOARD_V1 text with stable total/input/output/cache reader fields.
 * [Pos] Board-runtime token aggregation and dashboard payload producer.
 */
...
size_t runtime_stats_render_display(char *out, size_t out_size) {
  ...
  br_stats_format_compact_count(
    br_stats_clamp_nonneg(g_ctx.totals.input_tokens), input_str, sizeof(input_str));
  br_stats_format_compact_count(
    br_stats_clamp_nonneg(g_ctx.totals.output_tokens), output_str, sizeof(output_str));
  br_stats_format_compact_count(
    br_stats_clamp_nonneg(g_ctx.totals.cached_input_tokens), cache_str, sizeof(cache_str));
  ...
  br_snprintf_append(out, out_size, &used, "\nmetricUnit=TOKEN");
  /* Stable machine-readable lines keep the Token widget's three pages
     independent from localized prose in `breakdown`. */
  br_snprintf_append(out, out_size, &used, "\ntokenTotal=%s", total_str);
  br_snprintf_append(out, out_size, &used, "\ntokenInput=%s", input_str);
  br_snprintf_append(out, out_size, &used, "\ntokenOutput=%s", output_str);
  br_snprintf_append(out, out_size, &used, "\ntokenCache=缓存 %s", cache_str);
