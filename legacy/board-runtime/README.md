...
| `board-widget-runtime.py` | 解释 `.clawpkg` widget，生成黑底 `tool` 信息布局、coffee/timer/droplet/gauge 工具图标，以及 target/trophy/star/bolt/blocks/snake 和宽幅三洞 mole-* 游戏预置的负一屏 payload |
...
- `.active-widget`：当前启用组件 id。安装的 begin/chunk/commit 与删除共用
  `.widget-transaction.lock`，并用 `.incoming-widget-transfer/.incoming-widget-id`
  拒绝陈旧提交。每个 chunk 解码后校验 `decodedSize` 与 FNV-1a checksum，再以
  `transferId + path + index` 返回 `widget-install-ack`。`widget/delete` 会在锁内
  将目标包和 `.previous` 原子改名为隐藏 tombstone；只有删除当前启用组件时才清
  活动标记并切回 `main`，删除其他已安装组件不会打断当前页面。`widget/list`
  会在共享读锁下枚举 `widgets/<id>/component.json`，通过 `widget/inventory`
  返回板端真实安装列表、manifest 健康状态和当前启用 id。删除后会尽力回收
  tombstone 与 `.widget-state-<id>.json`；因此失败提交不能重新激活已删除组件，
  重试也保持幂等。

Widget 使用可选 dashboard 槽位和声明式有限状态机，不要求填满固定模板。
每个 `buttons.json` action/event 都必须唯一，确保 Pet Manager 能在组件内部逐动作换键；
内置小游戏和工具组件都使用 SW1/SW2/SW3 三枚实体键的短按动作；工具组件中，
番茄钟对应暂停/跳过/重置，喝水提醒对应确认/暂停/切页，Token 仪表盘对应
总量/输入/输出。组件中心当前提供这三款内置工具；摸鱼倒计时不再作为内置项陈列。
工具仍使用有界 `pixel` 契约，但固定走黑底 `tool` 信息布局，以一个大指标和单一
palette 强调色为中心，并使用 coffee/timer/droplet/gauge 等安全图标；小游戏继续
使用明亮舞台。没有自定义 CSS、SVG 或像素矩阵。旋钮长按固定绑定保留 action `page_main`；
Linux widget runtime 会原子写入 `.screen-page=main` 返回桌宠主体，这个系统 action
不需要 `runtime/widget.json` transition。
`tick` 除了 `set` / `inc`，还支持配对的
`when: {var, lte}` + `then: {to?, set?, inc?}`，可在倒计时归零后进入结果态，
用于点击计分、倒计时和有限轮次挑战等轻量小游戏；运行时仍不执行组件自带脚本。
带 `game` 字段的实时方块与贪吃蛇组件属于 ESP32-P4 `p4-bounded-v2`
能力，Linux runtime 会明确拒绝，而不是把动态游戏静默降级成静态卡片。
