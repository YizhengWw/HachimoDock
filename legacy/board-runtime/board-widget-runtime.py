...
fb-stats-renderer.py pick it up. Tick rules support the same bounded
``when: {var, lte}`` + ``then`` completion branch as the ESP32-P4 runtime,
which is enough for countdown challenges and other finite-state mini-games.
Real-time ``blocks`` and ``snake`` declarations are intentionally rejected here:
their fixed-grid engines are a P4-only ``p4-bounded-v2`` capability, so a Linux
board never degrades them into a misleading static card.
...
def validate_widget(spec: dict) -> None:
    ...
    _require(
        "game" not in spec,
        "runtime/widget.json game requires ESP32-P4 p4-bounded-v2",
    )
    ...
    for i, tk in enumerate(tick):
        ...
        when = tk.get("when")
        then = tk.get("then")
        _require((when is None) == (then is None),
                 f"tick[{i}].when and then must be provided together")
        if when is not None:
            _require(isinstance(when, dict) and set(when) == {"var", "lte"},
                     f"tick[{i}].when must contain only var + lte")
            condition_var = when.get("var")
            _require(condition_var in vars_decl
                     and vars_decl[condition_var].get("type") == "int",
                     f"tick[{i}].when.var must reference an int var")
            _require(isinstance(when.get("lte"), int),
                     f"tick[{i}].when.lte must be int")
            _require(isinstance(then, dict)
                     and set(then).issubset({"to", "set", "inc"})
                     and bool(then),
                     f"tick[{i}].then must change state or vars")
            if "to" in then:
                _require(then["to"] in state_set,
                         f"tick[{i}].then.to must be known state")
            for op in ("set", "inc"):
                if op in then:
                    _require(isinstance(then[op], dict),
                             f"tick[{i}].then.{op} must be object")
                    for k in then[op]:
                        _require(k in vars_decl,
                                 f"tick[{i}].then.{op} references unknown var {k}")
    ...
    visual_presets = {
        "visualStyle": {"classic", "pixel"},
        "visualPalette": {"candy", "sunset", "mint", "arcade"},
        "visualLayout": {"arcade", "scoreboard", "tool"},
        "visualSprite": {
            "target", "trophy", "star", "bolt", "coffee", "timer",
            "droplet", "gauge", "blocks", "snake",
    ...
    for slot, rule in dashboard.items():
        ...
        if slot in visual_presets:
            choices = []
            if isinstance(rule, str):
                choices = [rule]
            elif isinstance(rule, dict) and "switch_state" in rule:
                choices = list(rule["switch_state"].values())
            elif isinstance(rule, dict) and "switch_page" in rule:
                choices = list(rule["switch_page"].values())
            _require(choices and all(isinstance(value, str) for value in choices),
                     f"dashboard.{slot} must be a literal or state/page switch")
            _require(all(value in visual_presets[slot] for value in choices),
                     f"dashboard.{slot} contains an unsupported visual preset")
            continue
        if isinstance(rule, str):
            continue  # literal
...
def validate_buttons(buttons: list, transitions: list) -> None:
    ...
    _require(len(buttons) <= 8, "buttons.json supports at most 8 bindings")
    ...
    for i, b in enumerate(buttons):
        ...
        _require(
            b["action"] not in actions_in_buttons,
            f"buttons[{i}].action must be unique for independent remapping",
        )
        slots = (
            {"knob.rotate_cw", "knob.rotate_ccw"}
            if b["event"] == "knob.rotate_cw / knob.rotate_ccw"
            else {b["event"]}
        )
        _require(
            event_slots.isdisjoint(slots),
            f"buttons[{i}].event conflicts with an existing binding",
        )
        actions_in_buttons.add(b["action"])
        event_slots.update(slots)
...
class WidgetRuntime:
    ...
    def __init__(self, root: Path):
        ...
        self.screen_page_file = root / ".screen-page"
    ...
    def dispatch_action(self, action: str) -> bool:
        ...
        if action == "page_main":
            tmp = self.screen_page_file.with_suffix(".screen-page.tmp")
            tmp.write_text("main\n", encoding="utf-8")
            tmp.replace(self.screen_page_file)
            LOG.debug("reserved system action page_main returned to pet screen")
            return True
    ...
    def _apply_tick_rule(self, tk: dict) -> bool:
        changed = False
        ...
        for k, d in tk.get("inc", {}).items():
            self.vars[k] = self.vars.get(k, 0) + d
            changed = True
        when = tk.get("when")
        then = tk.get("then")
        if when is not None and self.vars.get(when["var"], 0) <= when["lte"]:
            if "to" in then and self.state != then["to"]:
                self.state = then["to"]
                changed = True
            for k, v in then.get("set", {}).items():
                if self.vars.get(k) != v:
                    self.vars[k] = v
                    changed = True
            for k, d in then.get("inc", {}).items():
                self.vars[k] = self.vars.get(k, 0) + d
                changed = True
        return changed
