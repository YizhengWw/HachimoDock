...
| key | maxBytes | 允许值 | 设备行为 |
|---|---:|---|---|
| `visualStyle` | 16 | `classic` / `pixel` | 经典信息卡或像素小游戏 |
| `visualPalette` | 16 | `candy` / `sunset` / `mint` / `arcade` | 内置配色 |
| `visualLayout` | 16 | `arcade` / `scoreboard` / `tool` | 游戏舞台或黑底工具信息布局 |
| `visualSprite` | 16 | `target` / `trophy` / `star` / `bolt` / `coffee` / `timer` / `droplet` / `gauge` / `blocks` / `snake` / `mole-ready` / `mole-left` / `mole-center` / `mole-right` | 内置像素精灵；工具图标与游戏场景共用安全词表 |

这些字段只选择设备固件内置视觉预置。包不能携带任意 CSS、SVG、脚本或像素矩阵。

未知 key 被解析器忽略。

这些槽位是 640×480 逻辑画布上的受控渲染原语，不是一张必须填满的模板。
组件应只声明内容真正需要的槽位；计时器、数据卡和小游戏可以各自采用不同的信息密度。
...
由 `claw-pet-manager/ref/src/lib/clawpkg-contract.js` 中的 `COMPONENT_DASHBOARD_V1_SLOTS` 定义同步。客户端、Linux runtime 与 P4 runtime 的 14 个槽位 id、maxBytes 和视觉词表必须严格一致；修改契约时必须同步各端。
