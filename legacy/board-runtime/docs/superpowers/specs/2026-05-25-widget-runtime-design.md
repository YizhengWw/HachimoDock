...
Let `petAgent-ui-generator` skill emit a fully working widget. Install → push to RPi → device actually runs the widget (button transitions, periodic tick, counter changes, multi-page, bounded mini-games, optional HTTP fetch / on-device file read). No per-widget code shipped from the LLM.

> 2026-07-23 amendment: dashboard slots are optional safe-render primitives rather
> than a mandatory visual template. The shared P4/Linux subset supports bounded
> tick completion with paired `when: {var, lte}` and
> `then: {to?, set?, inc?}`. Components own up to eight unique input bindings;
> the final P4 install confirmation warns before replacing the current hardware map.
...
| `tick` | no | array of `{every_ms, while_state?, set?, inc?, when?, then?}`. `every_ms ≥ 100`; `when` is limited to `{var, lte}` and must be paired with `then: {to?, set?, inc?}`. |
...
- No arbitrary conditionals or expressions. The only v1 condition is tick completion via `when: {var, lte}` paired with bounded `then` effects.
...
- `transitions[*].when: {var: x, gt: 5}` conditional guards (tick `lte` completion is already v1)
...
**widget.json transitions reference *action names* (`timer.start_pause`), not hardware events (`button.sw1.short_press`).** buttons.json is the only place that does the action↔hardware translation.

```json
[
  { "action": "timer.start_pause", "control": "SW1",      "event": "button.sw1.short_press",     "label": "开始/暂停" },
  { "action": "timer.reset",       "control": "SW1",      "event": "button.sw1.long_press",      "label": "重置" },
  { "action": "timer.bump_up",     "control": "前方旋钮", "event": "knob.rotate_cw",             "label": "调长" },
  { "action": "timer.bump_down",   "control": "前方旋钮", "event": "knob.rotate_ccw",            "label": "调短" },
  { "action": "nav.next_page",     "control": "屏幕区域", "event": "screen.region.tap",          "label": "切页" }
]
```

**Client override:** when user changes "开始/暂停" from SW1 to encoder press, the entry becomes `{action: "timer.start_pause", control: "前方旋钮", event: "button.encoder.short_press", label: "开始/暂停"}`. widget.json stays untouched. The final install step must warn that a P4 component binding replaces the current device button map; undeclared hardware events are disabled and screen swipe remains the system escape gesture.

**Runtime routing:**

```
hardware event (.current-event line, e.g. {control:"SW1", event:"button.sw1.short_press"})
...
8. user presses SW1 on the physical device
9. board-rotary-input writes {control:"SW1", event:"button.sw1.short_press"} to .current-event
