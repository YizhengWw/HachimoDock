...
| `input/config-query` | host -> board | 读取 `.button-config` 中的权威按钮映射；没有持久化文件时返回板端默认映射 |
| `input/config-state` | board -> host | 按 `requestId` 返回完整按钮配置，供 Pet Manager 打开页面或 USB 重连时刷新客户端缓存 |
